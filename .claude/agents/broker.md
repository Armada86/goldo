...
**`Telegram-buy`/`Telegram-sell` is not an algorithmic rule** — it's what `telegram_command_job.py`
stamps on a trade opened by hand via a Telegram command ("sell broker A"), not by any of the rules
below firing. If a trade's `rule_name` is one of these two, its `triggering_alerts` will read
`"Manual (Telegram command)"`, not a real alert list — don't try to explain it as if the usual
