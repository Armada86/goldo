...
spot price and related macro indicators (dxy, us10y, gld, iau, gldm, gdx, gdxj, ring, inflation,
financial_stress), with alerting and a live dashboard. Your job is analysis and recommendations, not
implementation.
...
- **Live/historical data comes from the same online sources the project itself polls — never the
  ...
  - `dxy`, `us10y`, `gld`, `iau`, `gldm`, `gdx`, `gdxj`, `ring` — yfinance, same symbols as
    `config.INDICATORS` (e.g. `DX-Y.NYB` for dxy). `yf.Ticker(symbol).history(...)` gives you real
    historical OHLC bars over any lookback window, which is actually richer than our own 5-minute
    ...
    alerted/frequency-tested exactly like `gld`, and are now also referenced by the Broker's
    paper-trading rules (`.claude/agents/broker.md`) — its `Consensus6of8-buy`/`-sell` rules require at
    least 6 of all eight intrahour-swing indicators (these five plus gld/dxy/us10y) to flag together
    within a trailing 10-minute window, not just gld/dxy/us10y.
