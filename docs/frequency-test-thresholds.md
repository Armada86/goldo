...
**These twenty-four thresholds are no longer a fixed, hand-picked table — they're re-tuned
automatically every weekday morning.** The values below are the ones this backtest produced at the time
this doc was written; the live values always live in `intrahour_swing_thresholds.json`, not here. See
"Weekday auto-tuning" below for how and how often they actually move.

IAU/GLDM and GDX/GDXJ/RING were added later than GLD/DXY/US10Y (same mechanism, same target, just newer
backtest dates — see their rows' notes below) so their entries reflect separate runs. SGOL was tracked
here briefly and removed (swapped for the gold-mining ETFs GDX/GDXJ/RING) before any weekday run ever
wrote a value for it, so no historical SGOL data exists anywhere in this project.
...
|---|---|---|---|
| GLD | $1.65 | $1.42 | $1.08 |
| IAU | $0.3450 | $0.3000 | $0.2251 |
| GLDM | $0.3650 | $0.3100 | $0.2400 |
| GDX | $0.8700 | $0.7425 | $0.6050 |
| GDXJ | $1.2150 | $1.0800 | $0.8500 |
| RING | $0.7600 | $0.6400 | $0.5050 |
...
Event counts at the picked thresholds, over the last 60 days (2026-07-19 to 2026-09-17 for
GLD/DXY/US10Y; 2026-06-25 to 2026-09-18 for the later IAU/GLDM/GDX/GDXJ/RING runs):
...
| GLD | 10 min | $1.42 | 60 |
| GLD | 5 min | $1.08 | 62 |
| IAU | 15 min | $0.3450 | 56 |
| IAU | 10 min | $0.3000 | 57 |
| IAU | 5 min | $0.2251 | 56 |
| GLDM | 15 min | $0.3650 | 57 |
| GLDM | 10 min | $0.3100 | 56 |
| GLDM | 5 min | $0.2400 | 59 |
| GDX | 15 min | $0.8700 | 56 |
| GDX | 10 min | $0.7425 | 56 |
| GDX | 5 min | $0.6050 | 57 |
| GDXJ | 15 min | $1.2150 | 56 |
| GDXJ | 10 min | $1.0800 | 56 |
| GDXJ | 5 min | $0.8500 | 57 |
| RING | 15 min | $0.7600 | 56 |
| RING | 10 min | $0.6400 | 56 |
| RING | 5 min | $0.5050 | 56 |
| DXY | 15 min | 0.102 | 58 |
| DXY | 10 min | 0.084 | 61 |
| DXY | 5 min | 0.064 | 62 |
...
All twenty-four land inside the 56–64 target band. US10Y's 15-min threshold sits on a sharp cliff in the
...
thresholds in `intrahour_swing_thresholds.json`. For any of the twenty-four indicator/window
combinations that has drifted outside `FREQUENCY_TEST_TARGET +/- FREQUENCY_TEST_TOLERANCE` (60±4
events/60 days), it
...
marking each one `unchanged` (with its threshold and current event count) or showing the change (old
threshold/count -> new threshold/count). Unlike the interactive workflow above, this path never asks
...
`us10y_10min`/`us10y_5min`, plus `iau_15min`/`iau_10min`/`iau_5min`/`gldm_15min`/`gldm_10min`/
`gldm_5min`/`gdx_15min`/`gdx_10min`/`gdx_5min`/`gdxj_15min`/`gdxj_10min`/`gdxj_5min`/`ring_15min`/
`ring_10min`/`ring_5min` (added later, nullable, so pre-existing rows logged before a given indicator
was tracked simply have `NULL` there). SGOL had columns here too, briefly, before being swapped for
GDX/GDXJ/RING — dropped via a manual `ALTER TABLE ... DROP COLUMN` migration, safe since no weekday run
had written to them yet. The single historical row (`2026-09-17`) was migrated by
`backfill_threshold_history.py` (a one-time script, safe to re-run — it skips if the table already has
rows), from before any of IAU/GLDM/GDX/GDXJ/RING existed, so it only ever populated the GLD/DXY/US10Y
columns.
