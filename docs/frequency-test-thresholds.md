...
**These eighteen thresholds are no longer a fixed, hand-picked table — they're re-tuned automatically
every weekday morning.** The values below are the ones this backtest produced at the time this doc was
...
IAU/GLDM/SGOL were added later than GLD/DXY/US10Y (same mechanism, same target, just a newer backtest
date — see their rows' notes below) so their entries reflect a separate run.
...
GLD/DXY/US10Y; 2026-06-25 to 2026-09-18 for the later IAU/GLDM/SGOL run):
...
| SGOL | 15 min | $0.1700 | 62 |
| SGOL | 10 min | $0.1500 | 57 |
| SGOL | 5 min | $0.1150 | 59 |
| DXY | 15 min | 0.102 | 58 |
| DXY | 10 min | 0.084 | 61 |
| DXY | 5 min | 0.064 | 62 |
...
All eighteen land inside the 56–64 target band. US10Y's 15-min threshold sits on a sharp cliff in the
...
Every one of these eighteen threshold values is wired into `rules.check_intrahour_swing_alerts` and
alerts to Telegram — no window or indicator here is backtest-only. A poll can produce anywhere from
zero to eighteen of these alerts (three windows x six indicators) in a single cycle, each rising-edge
deduped per window so a sustained swing alerts once, not repeatedly for the rest of the window. IAU,
GLDM, and SGOL are alerted and re-tuned exactly like GLD, but — unlike GLD — are **not** referenced by
the Broker's paper-trading rules (`.claude/agents/broker.md`), which still only watch GLD/DXY/US10Y.
...
thresholds in `intrahour_swing_thresholds.json`. For any of the eighteen indicator/window combinations
that has drifted outside `FREQUENCY_TEST_TARGET +/- FREQUENCY_TEST_TOLERANCE` (60±4 events/60 days), it
...
A Telegram message is sent every weekday run either way, listing all eighteen combinations and marking
each one `unchanged` (with its threshold and current event count) or showing the change (old
...
Every weekday run also logs one row — the date and that run's final value for all eighteen
...
`gldm_5min`/`sgol_15min`/`sgol_10min`/`sgol_5min` (added later, nullable, so pre-existing rows logged
before IAU/GLDM/SGOL were tracked simply have `NULL` there). The single historical row (`2026-09-17`)
was migrated by `backfill_threshold_history.py` (a one-time script, safe to re-run — it skips if the
table already has rows), from before IAU/GLDM/SGOL existed, so it only ever populated the
GLD/DXY/US10Y columns.
