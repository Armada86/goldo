...
National Employment Change report, released a couple of days before BLS NFP each month (next print
expected Monday) — it is tracked as a FRED indicator here but does not (yet) have its own
Postgres release-data table or `docs/fundamental-analyst-*.md` log the way BLS NFP does.
