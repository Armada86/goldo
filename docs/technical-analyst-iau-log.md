...
IAU alerts via `rules.check_intrahour_swing_alerts` — trailing 15/10/5-minute high-low swing, each
window independently thresholded (`INTRAHOUR_SWING_ALERT_THRESHOLD["iau"]`). The live threshold values
...
swing size, threshold, and current price. Unlike GLD, IAU is **not** wired into the Broker's automated
